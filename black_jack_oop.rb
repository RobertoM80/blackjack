...
module Hand
  ...
  def total
    ...
    rank.select {|value| value == 'A'}.count.times do
      break if total <= 21
      total -= 10
    end
    total
...
class Blackjack
  ...
  def player_turn
    ...
    while !player.is_busted?
      puts "What do you want to do? 1) hit or 2) stay"
      answer = gets.chomp

      if !["1", "2"].include?(answer)
        puts "You must enter 1 or 2. Try again.."
        next
      end
  ...
  def dealer_turn
    ...
    puts "Dealer's second card was #{dealer.cards[0]}."
  ...
  def play_again?
    puts ""
    puts "Would you like to play again? press 1 for yes..."
    if gets.chomp == '1'
